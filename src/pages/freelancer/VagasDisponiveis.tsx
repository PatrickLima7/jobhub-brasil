...
export default function VagasDisponiveis() {
  ...
  const filtered = useMemo(() => {
    ...
    // Calculate distances
    if (userLocation && geoActive) {
      result = result.map(j => ({ ...j, distance: undefined }));
    }

    if (sortBy === 'valor') {
      result = [...result].sort((a, b) => b.valor - a.valor);
    } else if (sortBy === 'proximas' && userLocation) {
      // Jobs without coords go to end
      result = [...result].sort(() => 0);
    }

    return result;
  }, [jobs, search, selectedFuncoes, cidade, dataMinima, sortBy, userLocation, geoActive]);
